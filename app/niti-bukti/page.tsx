  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        // 3. Simpan catatan ke tabel submission_aksi dengan siswa_id yang valid
        ...
        // 4. Update progress_siswa untuk bukti dan buka Niti Bakti
        ...
        const { data: baktiProg } = await supabase
          .from("progress_siswa")
          .select("status")
          .eq("siswa_id", profile.id)
          .eq("tahap_niti", "bakti")
        ...
        if (baktiProg && baktiProg.status === "terkunci") {
          await supabase
            .from("progress_siswa")
            .update({
              status: "tersedia",
