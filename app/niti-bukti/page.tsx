...
export default function NitiBuktiPage() {
      ...
      if (sub) {
        if (sub.file_url) {
          ...
          try {
            ...
            // pathname format: "/submissions/bukti/file.pdf"
            // substring(1) removes the leading slash -> "submissions/bukti/file.pdf"
            setUploadedPath(decodeURIComponent(urlObj.pathname.substring(1)));
          } catch (e) {
            console.error("Gagal parse URL:", e);
          }
        } else {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        // 1. Dapatkan Presigned URL dari R2 (disimpan di subfolder submissions/bukti/)
        ...
        // Menggunakan upsert agar jika siswa sudah pernah submit, data akan di-update (bukan error duplikat)
        const { error: insertErr } = await supabase.from("submission_aksi").upsert(
          {
            siswa_id: profile.id,
            tahap_niti: "bukti",
            kategori_aksi: null,
          ...
          { onConflict: "siswa_id,tahap_niti" }
        );
  ...
  const handleCancel = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (uploadedPath && profile?.id) {
      try {
        await deleteFileFromR2(uploadedPath);
        // Hapus nama file dan URL dari database agar tidak muncul lagi saat direfresh
        await supabase
          .from("submission_aksi")
          .update({ nama_file: null, file_url: null })
          .eq("siswa_id", profile.id)
          .eq("tahap_niti", "bukti");
      } catch (err) {
  ...
  const executeCancelSubmission = async () => {
    ...
    try {
      ...
      if (!pathToDel && latestSub?.file_url) {
        try {
          const urlObj = new URL(latestSub.file_url);
          pathToDel = decodeURIComponent(urlObj.pathname.substring(1));
        } catch (e) {
          console.error("Gagal parse URL untuk hapus file:", e);
        }
      }
